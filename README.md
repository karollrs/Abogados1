...
## ✅ Base de datos en Convex (Nueva)

Este repositorio ya está preparado para usar **Convex** como base de datos principal para:
- leads
- call logs
- attorneys
- users

### 1) Crear/arrancar Convex
...
Eso te va a:
- crear tu proyecto Convex (local o con cuenta)
- generar `convex/_generated/*`
- darte una URL tipo `CONVEX_URL=...`

### 2) Configurar variables de entorno
Crea un archivo `.env` en la raíz (o usa `.env.local`) y pon:

```env
CONVEX_URL=TU_CONVEX_URL
SESSION_SECRET=dev-secret-change-me
```

> **Importante:** este proyecto mantiene Express para auth + webhooks, y ahora el módulo `server/storage.ts` guarda/lee datos desde **Convex**.
...
Una vez corriendo el backend, ejecuta el bootstrap:

```bash
curl -X POST http://127.0.0.1:5000/api/auth/bootstrap \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@demo.com","name":"Admin","password":"Admin1234"}'
```
...
### 2) Configurar variables de entorno
Crea un archivo `.env` en la raíz (o completa el tuyo) con:

```env
CONVEX_URL=PEGA_AQUI_TU_CONVEX_URL
SESSION_SECRET=una_clave_larga
```

> La `CONVEX_URL` te la muestra el comando `npm run convex:dev`.

### 3) Levantar el CRM
En otra terminal (sin cerrar Convex):
...
- Backend: `http://127.0.0.1:5000`
- Frontend (Vite): `http://127.0.0.1:5000` (sirve Vite en dev a través del backend)

### 4) Crear el primer admin (bootstrap)
Si no tienes usuarios creados aún:

```bash
POST http://127.0.0.1:5000/api/auth/bootstrap
{
